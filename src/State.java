...
*/
    ...
    public List<State> generateChildrenInformed(State goal, Set<State> visited) {
        ...
        // If the current state has all blocks in one stack, do not expand it
        if (isSingleStackState() ) {
            return children; // No children since we reached a terminal-like state
        }
        ...
        for (int i = 0; i < state.size(); i++) {
            ...
            for (int j = 0; j < state.size(); j++) {
                ...
                // Check for redundant moves (e.g., A -> B -> A)
                if (!visited.contains(childState) && !isRedundantMove(newStacks, i, j)) {
                    // Calculate the heuristic value for the child state
                    double heuristic = Problem.calculateHeuristic(childState, goal);
                    childState.setH(heuristic);

                    // Optionally, set other costs for A* (if applicable)
